...
Sin ninguna columna del dataset escrita a mano.

El caso invalido se DERIVA del contrato: se busca una feature categorica y se
le manda un valor que su lista de permitidos no incluye. La version del
template mandaba Neighborhood="Polanco" para provocar un 400; con otro modelo
esa clave ni siquiera existe en el contrato, la peticion resulta VALIDA, y el
test falla por una razon que no tiene nada que ver con lo que mide.
